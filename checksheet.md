...
| 25 | 🟥 高  | インフラ | 利用しているWebサーバ/アプリケーションサーバの種類及びバージョンについてご教示ください。 |
...
| 54 | 🟧 中  | 情報セキュリティ | 情報セキュリティ責任者を配置し、セキュリティポリシーの策定を行なっているか。 |
| 55 | 🟧 中  | 情報セキュリティ | 従業員に対して定期的にセキュリティ教育を実施しているか。 |
| 56 | 🟧 中  | 情報セキュリティ | 社内での個人情報の取り扱いについて、明確なルールを定めているか。 |
| 57 | 🟥 高  | 情報セキュリティ | 貴社で利用している社内用サーバ及びSaaSの一覧についてご教示ください。 |
| 58 | 🟧 中  | 情報セキュリティ | 各種アカウント発行について、適切な申請・承認プロセスが存在するか。 |
| 59 | 🟩 低  | 情報セキュリティ | 各種アカウントについて、定期的な棚卸しは実施されているか。 |
| 60 | 🟧 中  | 情報セキュリティ | パスワードポリシーを適切に定義しているか。 |
| 61 | 🟩 低  | 情報セキュリティ | 各サービスについて、設定可能なものは多要素認証を設定しているか。 |
| 62 | 🟥 高  | 情報セキュリティ | ウイルス対策ソフト、EDRなどの端末保護ツールは導入されているか。導入されている場合は製品名をご教示ください。 |
| 63 | 🟧 中  | 情報セキュリティ | Microsoft Entra ID、OktaなどのID管理ツールは導入されているか。導入されている場合は製品名をご教示ください。 |
| 64 | 🟩 低  | 情報セキュリティ | 各デバイスにMDMは導入されているか。導入されている場合は製品名をご教示ください。 |
| 65 | 🟥 高  | 情報セキュリティ | 社内ネットワークは社外から適切に分離され、社外からアクセスする場合はVPNなどを通してアクセスする運用になっているか。 |
| 66 | 🟥 高  | 情報セキュリティ | 社内ネットワークと外部ネットワークとの境界にファイアウォールなどは設置されているか。 |
| 67 | 🟥 高  | 情報セキュリティ | Wi-Fi接続において、WEP、WPA1などの古いプロトコルを使用していないか。 |
| 68 | 🟧 中  | 情報セキュリティ | ISMS、Pマークなどは取得しているか。 |
| 69 | 🟧 中  | 情報セキュリティ | 業務委託先の選定において、セキュリティ的な観点からチェックが行われているか。 |
